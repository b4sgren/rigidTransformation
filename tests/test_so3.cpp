...
TEST_F(SO3_Fixture, TestFromRPYAngles) {
    for (int i{0}; i != 100; ++i) {
        ...
        Eigen::Matrix3d R_true =
            // yawAngle.matrix() * pitchAngle.matrix() * rollAngle.matrix();
            rollAngle.matrix() * pitchAngle.matrix() * yawAngle.matrix();
...
TEST_F(SO3_Fixture, TestFromAxisAngle) {
    for (int i{0}; i != 100; ++i) {
        ...
        Eigen::Matrix3d R_true =
            Eigen::AngleAxisd(phi, vec / vec.norm()).matrix();
